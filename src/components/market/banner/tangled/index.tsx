...
import bannerImages from '@components/market/banner/tangled/banner-image.json';
import moment from 'moment-timezone';
import { TimeFormat } from '@utils/formatter/time';
import { css } from '@emotion/core';
import { useTranslation } from 'next-i18next';

export type MarketBannerTangledProps = {
  remain?: number;
}

export const MarketBannerTangled = ({ remain }: MarketBannerTangledProps) => {
    ...
    <Container>
      <Content>
        <Header>
          ...
          <Description>{t('hero.tangled.desc', { ns: 'marketplace' })}</Description>
          <Title>Tangled Timepieces</Title>
        </Header>
            ...
            <Column className={classNames({ reverse: index % 2 === 0 })} key={index}>
              <Rolling className={classNames({ reverse: index % 2 === 0 })}>
                {images.map((image, index) => (
                  // eslint-disable-next-line react/no-array-index-key
                  <Box key={index}>
                    <Image src={image} alt="" layout="responsive" width="100%" height="100%" loader={NileCDNLoader} />
                  </Box>
                ))}
              </Rolling>
            </Column>
...
MarketBannerTangled.defaultProps = {
  remain: undefined,
};
