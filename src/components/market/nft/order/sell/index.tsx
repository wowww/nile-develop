...
export const MarketNftOrderSell = ({ ...props }: MarketNftMintingProps) => {
  const [form] = Form.useForm();
  const [result, setResult] = useState<string>();
  const [loading, setLoading] = useState(false);
  const { login, wallet, contracts, web3 } = useWeb3Provider();

  const register = useCallback(({ orderType, order }) => {
    const contract = contracts?.CurateMarket;
    setLoading(true);
    const data: Order = { ...order, buyer: [], soldAmount: [], price: web3?.utils.toWei(String(order.price)) };

    contract?.registerSellOrder(2, data)?.then((response) => {
  ...
  return (
    <Form onFinish={register} form={form}>
      <Typography.Title>NFT 판매 등록</Typography.Title>
      ...
      <Form.Item name={['order', 'orderID']} label="주문 아이디(Order ID)" initialValue={0}>
        <InputNumber controls={false} />
      </Form.Item>
      <Form.Item name={['order', 'orderAmount']} label="주문 수량" initialValue={1}>
        <InputNumber controls={false} />
      </Form.Item>
      <Form.Item name={['order', 'startTime']} label="경매 시작 시간" initialValue={moment().add(60, 'm').unix()}>
        <InputNumber controls={false} />
      </Form.Item>
